...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-simple-2fa runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-simple-2fa.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_simple_2fa")
...
def lint(c):
    ...
    """
    c.run("flake8 django-simple-2fa tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_simple_2fa.__version__))
    c.run("git push --tags")
